...
//done defining, the new things start on line 180 or so
...
units::volume stomach_contents::capacity( const Character &owner ) const
{
    ...
    //elseif because they're mutually exclusive, that way we save a lot of uneccesary checks -ungen
    if( owner.has_trait( trait_SMALL_OK ) ) {
        max_mod *= 0.5;
    }
    elseif( owner.has_trait( trait_SMALL2 ) ) {
        max_mod *= 0.5;
    }
    elseif( owner.has_trait( trait_SMALL ) ) {
        max_mod *= 0.75;
    }
    elseif( owner.has_trait( trait_LARGE ) ) {
        max_mod *= 1.5;
    }
    elseif( owner.has_trait( trait_LARGE_OK ) ) {
        max_mod *= 1.5;
    }
    elseif( owner.has_trait( trait_HUGE ) ) {
        max_mod *= 2;
    }
    elseif( owner.has_trait( trait_HUGE_OK ) ) {
        max_mod *= 2;
    }
